...
int16_t calcChargeCurrentCutOff(int16_t u16_lChargeCurrent)
{
  ...
  uint16_t u16_lCutOffStartVoltage = (uint16_t)WebSettings::getInt(ID_PARAM_INVERTER_CHARGE_CURRENT_CUT_OFF_START_VOLTAGE,0,DT_ID_PARAM_INVERTER_CHARGE_CURRENT_CUT_OFF_START_VOLTAGE);
  ...
  else
  {
    ...
    if(u16_lCutOffStartVoltage>0) // Wenn eine Startvoltage eingestellt ist
    {
      if(fl_lTotalCurrent<fl_lCutOffCurrent && getMaxCellSpannungFromBms()>=u16_lCutOffStartVoltage) u16_mChargeCurrentCutOfTimer++;
