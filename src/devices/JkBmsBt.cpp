...
#if 0
void jkBmsBtDecodeCellInfo_jk02(uint8_t devNr, uint8_t* pData, size_t length, uint8_t frameNr)
{
  ...
  if(frameNr==1)
  {
    ...
    setBmsAvgVoltage(devNr,getJk16bit(58 + u8_lOffset));

    // 60    2   0x00 0x00              Delta Cell Voltage    0.001        V
    setBmsMaxCellDifferenceVoltage(devNr,getJk16bit(60 + u8_lOffset));

    // 62    1   0x00                   Max voltage cell      1
    //data[62 + u8_lOffset] + 1
    
    // 63    1   0x00                   Min voltage cell      1
    //data[63 + u8_lOffset] + 1
  ...
  {  
    ...
    if(u8_frameVersion==FRAME_VERSION_JK02_32S)u8_lFrameOffset=112;

    // 112   2   0x00 0x00              Unknown112    (32S:112+32=144)
    u16_lPos=112-u8_lFrameOffset;
    if (u8_frameVersion == FRAME_VERSION_JK02_32S)
    {    
      setBmsTempature(devNr,0,(float)((int16_t) getJk16bit(u16_lPos + u8_lOffset))*0.1f);
      #ifdef JK_BT_DEBUG
      ESP_LOGI(TAG,"MOS Temp: %f°C",(float)((int16_t) getJk16bit(u16_lPos + u8_lOffset))*0.1f);
      #endif
    }
    ...
    u16_lPos=118-u8_lFrameOffset;
    float fl_lTotalVoltage = (float)getJk32bit(u16_lPos+u8_lOffset)*0.001f;
  ...
  if(frameNr==2 && u8_frameVersion==FRAME_VERSION_JK02)
  {
    uint8_t u8_lFrameOffset=128;
    // 126   4   0x00 0x00 0x00 0x00    Charge current        0.001        A
    //float current = (float) ((int32_t) getJk32bit(u16_lPos + u8_lOffset)) * 0.001f;
    float fl_lCurrent = (float) (((uint32_t(pData[1]) << 24) | (uint32_t(pData[0]) << 16) | 
      (uint32_t(u8_mRecvData[devNr][1]) << 8) | (uint32_t(u8_mRecvData[devNr][0]) << 0))*0.001f);
    ...
    u16_lPos=130-u8_lFrameOffset;
    setBmsTempature(devNr,1,(float) ((int16_t) getJk16bit(u16_lPos + u8_lOffset))*0.1f);

    // 132   2   0xBF 0x00              Temperature Sensor 2  0.1          °C
    u16_lPos=132-u8_lFrameOffset;
    setBmsTempature(devNr,2,(float) ((int16_t) getJk16bit(u16_lPos + u8_lOffset))*0.1f);

    // 134   2   0xD2 0x00              MOS Temperature       0.1          °C
    u16_lPos=134-u8_lFrameOffset;
    if (u8_frameVersion == FRAME_VERSION_JK02_32S)
    {
      uint32_t u6_lRawErrorsBitmask = (uint32_t)((uint16_t(pData[u16_lPos + u8_lOffset]) << 8) | (uint16_t(pData[u16_lPos + 1 + u8_lOffset]) << 0));
      setBmsErrors(devNr,u6_lRawErrorsBitmask);
    }
    else
    {
      setBmsTempature(devNr,0,(float)((int16_t) getJk16bit(u16_lPos + u8_lOffset))*0.1f);
    ...
    //                                    Cell count is not equal to settings
    u16_lPos=136-u8_lFrameOffset;
    if (u8_frameVersion != FRAME_VERSION_JK02_32S)
    {
      uint32_t u6_lRawErrorsBitmask = (uint16_t(pData[u16_lPos + u8_lOffset]) << 8) | (uint16_t(pData[u16_lPos + 1 + u8_lOffset]) << 0);
      setBmsErrors(devNr,u6_lRawErrorsBitmask);
    }

    // 138   2   0x00 0x00              Balance current      0.001         A
    u16_lPos=138-u8_lFrameOffset;
    setBmsBalancingCurrent(devNr,(float)((int16_t) getJk16bit(u16_lPos + u8_lOffset)) * 0.001f);

    // 140   1   0x00                   Balancing action                   0x00: Off
    //                                                                     0x01: Charging balancer
    //                                                                     0x02: Discharging balancer
    u16_lPos=140-u8_lFrameOffset;
    boolean bo_lBalancingAction = (bool)(pData[u16_lPos + u8_lOffset] != 0x00);

    // 141   1   0x54                   State of charge in   1.0           %
    u16_lPos=141-u8_lFrameOffset;
    setBmsChargePercentage(devNr,pData[u16_lPos + u8_lOffset]);

    // 142   4   0x8E 0x0B 0x01 0x00    Capacity_Remain      0.001         Ah
    u16_lPos=142-u8_lFrameOffset;
    //(float) getJk32bit(u16_lPos + u8_lOffset) * 0.001f;
    //ToDo: only mqtt

    // 146   4   0x68 0x3C 0x01 0x00    Nominal_Capacity     0.001         Ah
    u16_lPos=146-u8_lFrameOffset;
    //(float) getJk32bit(u16_lPos + u8_lOffset) * 0.001f;
    //ToDo: only mqtt

  }
  if(frameNr==3 && u8_frameVersion==FRAME_VERSION_JK02)
  {
    uint8_t u8_lFrameOffset=150;
    // 150   4   0x00 0x00 0x00 0x00    Cycle_Count          1.0
    u16_lPos=150-u8_lFrameOffset;
    //(float) getJk32bit(u16_lPos + u8_lOffset);
    //ToDo: only mqtt
    

    // 154   4   0x3D 0x04 0x00 0x00    Cycle_Capacity       0.001         Ah
    u16_lPos=154-u8_lFrameOffset;
    //(float) getJk32bit(u16_lPos + u8_lOffset) * 0.001f;
    //ToDo: only mqtt
    ...
    u16_lPos=162-u8_lFrameOffset;
    //(float) getJk32bit(u16_lPos + u8_lOffset);
    //format_total_runtime_(getJk32bit(u16_lPos + u8_lOffset));
    //ToDo: only mqtt

    // 166   1   0x01                   Charging switch enabled                      0x00: off, 0x01: on
    u16_lPos=166-u8_lFrameOffset;
    //(bool) data[u16_lPos + u8_lOffset]);
    //ToDo: only mqtt

    // 167   1   0x01                   Discharging switch enabled                   0x00: off, 0x01: on
    u16_lPos=167-u8_lFrameOffset;
    //(bool) data[u16_lPos + u8_lOffset]);
    //ToDo: only mqtt
...
void jkBmsBtDecodeCellInfo_jk02(uint8_t devNr, uint8_t* pData, uint8_t frameNr, uint8_t u8_frameVersion)
{
  ...
  if(frameNr==2 && u8_frameVersion==FRAME_VERSION_JK02 || (frameNr==3 && u8_frameVersion==FRAME_VERSION_JK02_32S))
  {
    ...
    if(u8_frameVersion==FRAME_VERSION_JK02)
    {
      fl_lCurrent = (float) (((uint32_t(pData[1]) << 24) | (uint32_t(pData[0]) << 16) | 
        (uint32_t(u8_mRecvData[devNr][1]) << 8) | (uint32_t(u8_mRecvData[devNr][0]) << 0))*0.001f);
...
#if 0
void jkBmsBtDecodeCellInfo_jk04(const std::vector<uint8_t> &data) {
  auto getJk16bit = [&](size_t i) -> uint16_t { return (uint16_t(data[i + 1]) << 8) | (uint16_t(data[i + 0]) << 0); };
  auto getJk32bit = [&](size_t i) -> uint32_t {
    return (uint32_t(getJk16bit(i + 2)) << 16) | (uint32_t(getJk16bit(i + 0)) << 0);
  };
  ...
  uint8_t cells = 24;
  float u16_lMinCellVoltage = 100.0f;
  float u16_lMaxCellVoltage = -100.0f;
  float fl_lTotalVoltage = 0.0f;
  uint8_t min_voltage_cell = 0;
  uint8_t max_voltage_cell = 0;
  for (uint8_t i = 0; i < cells; i++) {
    float u16_lCellVoltage = (float) ieee_float_(getJk32bit(i * 4 + 6));
    float u16_lCellResistance = (float) ieee_float_(getJk32bit(i * 4 + 102));
    fl_lTotalVoltage = fl_lTotalVoltage + u16_lCellVoltage;
    if (u16_lCellVoltage > 0 && u16_lCellVoltage < u16_lMinCellVoltage) {
      u16_lMinCellVoltage = u16_lCellVoltage;
    ...
    this->publish_state_(this->cells_[i].cell_voltage_sensor_, u16_lCellVoltage);
    this->publish_state_(this->cells_[i].cell_resistance_sensor_, u16_lCellResistance);
  }

  this->publish_state_(this->min_cell_voltage_sensor_, u16_lMinCellVoltage);
  this->publish_state_(this->max_cell_voltage_sensor_, u16_lMaxCellVoltage);
  this->publish_state_(this->max_voltage_cell_sensor_, (float) max_voltage_cell);
  this->publish_state_(this->min_voltage_cell_sensor_, (float) min_voltage_cell);
  this->publish_state_(this->total_voltage_sensor_, fl_lTotalVoltage);

  // 202   4   0x03 0x95 0x56 0x40    Average Cell Voltage               V
  this->publish_state_(this->average_cell_voltage_sensor_, (float) ieee_float_(getJk32bit(202)));

  // 206   4   0x00 0xBE 0x90 0x3B    Delta Cell Voltage                 V
  this->publish_state_(this->delta_cell_voltage_sensor_, (float) ieee_float_(getJk32bit(206)));
  ...
  this->publish_state_(this->balancing_current_sensor_, (float) ieee_float_(getJk32bit(222)));
  ...
  this->publish_state_(this->total_runtime_sensor_, (float) getJk32bit(286));
  this->publish_state_(this->total_runtime_formatted_text_sensor_, format_total_runtime_(getJk32bit(286)));
